...
public class EclipseCollectionsSolver {
    ...
    private MutableObjectIntMap<Cell> findHiddenSinglesInGroup(Map<Cell, MutableIntSet> cellsCandidates, Group group) {
        MutableIntObjectMap<List<Cell>> candidateCellsForNumber = IntObjectMaps.mutable.empty();
        group.getCells()
                .stream()
                .filter(Cell::isEmpty)
                .forEach(cell -> {
                    cellsCandidates.get(cell).forEach(candidate -> {
                        List<Cell> cells = candidateCellsForNumber.get(candidate);
                        if (cells == null) {
                            cells = new ArrayList<>();
                            candidateCellsForNumber.put(candidate, cells);
                        }
                        cells.add(cell);
                    });
                });
        MutableObjectIntMap<Cell> hiddenSingles = ObjectIntMaps.mutable.empty();
        candidateCellsForNumber.forEachKeyValue((number, cells) -> {
            if (cells.size() == 1) {
                hiddenSingles.put(cells.get(0), number);
            }
        });
        return hiddenSingles;
    }
